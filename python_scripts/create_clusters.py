...
orig_data_dir='/work/bb1153/b380883/TWP'
orig_data_path=''.join([orig_data_dir,'scream_dp_RCE_SMALL_al.eam.h1.2000-01-01-00000.nc'])

# Directory for output of clustering
dir_loc='/home/disk/eos15/smturbev/dpscream/'
# Figures location
fig_dir='/home/disk/eos15/smturbev/dpscream/plots/'
run_name='FirstTest'  #for naming figures

time_stride=1      # assumes you want every snapshot of data
time_slices=337    # of timeslices to count up to
eps_width=0.1
...
for i in np.arange(time_slices):   #loop through time steps
    ...
    lon_array_mean=np.nanmean(lon_array)
    lon_array_std=np.nanstd(lon_array)

    lat_array_mean=np.nanmean(lat_array)
    lat_array_std=np.nanstd(lat_array)
