...
GRID_IFS=/work/ka1081/DYAMOND_WINTER/ECMWF/IFS-4km/DW-CPL/atmos/fx/gn/ifs-4km-grid.nc
...
declare -a VarArray15min=(rltacc rltcsacc rstacc rstcsacc) # done scream: clivi rlt rst; done geos: rlut rst rsut clivi 
...
for v in "${VarArray15min[@]}"; do
    for d in "${DateArray[@]}"; do
        for f in $IN_PATH/$MODEL_PATH/DW-ATM/atmos/15min/$v/r1i1p1f1/2d/gn/*_20200$d*; do #for SHiELD 2d -> pl
            ...
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 -setgrid,$GRID_SCR -selgrid,1 $f $out_file
            ## GEOS, NICAM, SAM, UM
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 $f $out_file
            # # ARP
            # cdo -f nc4 -P 8 -s -w -divc,-900 -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 -setgrid,$GRID_ARP $f $out_file
            # # ICON
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 -setgrid,$GRID_ICO $f $out_file
            # # IFS
            cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 -setgrid,$GRID_IFS $f $out_file
        done
    done
done
...
#             ## SHiELD
#             # 1deg
#             # cdo -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r360x180 -setgrid,$GRID_SHi -selgrid,1 $f $out_file
#         done
#     done
# done
