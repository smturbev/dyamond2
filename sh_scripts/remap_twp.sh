...
declare -a VarArray15min=(rsut) # done scream: clivi rlt rst; done geos: rlut rst rsut clivi 
...
for v in "${VarArray15min[@]}"; do
    for d in "${DateArray[@]}"; do
        for f in $IN_PATH/$MODEL_PATH/DW-ATM/atmos/15min/$v/r1i1p1f1/2d/gn/*_20200$d*; do #for SHiELD 2d -> pl
            ...
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r3600x1800 -setgrid,$GRID_SCR -selgrid,1 $f $out_file
            ## GEOS, NICAM, SAM, UM
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r3600x1800 $f $out_file
            ## ARP
            # cdo -f nc4 -P 8 -s -w -divc,900 -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r3600x1800 -setgrid,$GRID_ARP $f $out_file
            ## ICON
            # cdo -f nc4 -P 8 -s -w -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r3600x1800 -setgrid,$GRID_ICO $f $out_file
        done
    done
done
...
for v in "${VarArray15min[@]}"; do
    for d in "${DateArray[@]}"; do
        for f in $IN_PATH/$MODEL_PATH/DW-ATM/atmos/15min/$v/r1i1p1f1/pl/gn/*_20200$d*; do #for SHiELD 2d -> pl
            ...
            ## SHiELD
            # 0.1deg
            cdo -sellonlatbox,$LON0,$LON1,$LAT0,$LAT1 -remapcon,r3600x1800 -setgrid,$GRID_SHi -selgrid,1 $f $out_file
        done
    done
done
...
# cdo -remapcon,/home/b/b380883/dyamond2/sh_scripts/REMAP_GEOS.txt $TWP/TWP_GEOS_qfvi_20200130-20200228.nc $TWP/TWP_GEOSr0.1deg_qfvi_20200130-20200228.nc
